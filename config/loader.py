...
class NLIConfig(BaseModel):
    """Configuration for NLI backend."""

    backend: Literal["http", "direct", "mock"] = "direct"
    model: Optional[str] = None
    api_key: Optional[str] = None
    base_url: Optional[str] = None
    url: Optional[str] = None  # For HTTP backend


class LettuceConfig(BaseModel):
    """Configuration for Lettuce backend."""
...
def load_config_from_env() -> ProfileConfig:
    ...
    # NLI config - default to HTTP for backward compatibility
    nli_url = os.environ.get("NLI_SERVICE_URL", "http://localhost:9000")
    nli = NLIConfig(
        backend="http",
        url=nli_url,
    )

    # Lettuce config - default to HTTP for backward compatibility
    lettuce_url = os.environ.get("LETTUCE_SERVICE_URL", "http://localhost:8000")
    lettuce = LettuceConfig(
        backend="http",
        url=lettuce_url,
    )
...
def load_config(
    ...
    Args:
        ...
        config_path: Path to config file. If None, uses config/models.yaml
                    in the project root.
